...
class AConfirmInformationView extends Component {
  ...
  gotoNext(){
    ...
    Alert.alert('提示', '请确认信息是否完整无误，提交后无法修改。' ,[{
            ...
            onPress : () => {
              if (index == 0) {
                that.props.navigation.navigate('AccidentConditionView',{index:index});
              } else {
                that.props.navigation.navigate('AAccidentFactAndResponsibilityView',{index:index});
              }
            }
          }])
  }
  ...
  renderOneParty(value,index) {
    return (
      <View style={{backgroundColor:'#ffffff'}} key={index}>
        ...
        <View style={{marginLeft:15,width:W-30,height:75,backgroundColor:'#D4D4D4',marginTop:10,marginBottom:15}}>
        </View>
      </View>
    )
  }
...
module.exports.AConfirmInformationView = connect()(AConfirmInformationView)
